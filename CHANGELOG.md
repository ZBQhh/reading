...
### 移动端 UI/UX 体验与零溢出深度重构

- **自选与期刊标题操作栏弹性网格重构**：
  - 彻底解决移动端（≤640px 及 320px 超窄屏）下「▾ 收起自选」、「⇅ 最新在前」、「⤓ 导出全部自选」等按钮组横向撑破溢出屏幕的问题。
  - 标题栏在手机端自动分行（标题独占首行，操作按钮在下方以弹性 2 列均分排布，底行单按钮自适应铺满），彻底消除横向滚动条。
- **全系弹窗与组件响应式深度适配**：
  - 导出全部弹窗（`.manual-export-card`）、生词本弹窗（`.wordbook-card`）与高亮回顾弹窗在移动端自动适配视口宽度（`100vw - 24px`），优化内边距与触摸安全圆角。
  - 书架分区（`.shelf-section`）移动端内边距由 22px 收紧至 12px~14px，卡片封面与元数据标签紧凑优雅展示。
- **触控热区与多端布局审计**：
  - 增大移动端返回键、翻页按钮与目录按钮触控区域（最低 38px~42px），提升单手触控精度。
  - 修复 iPad Pro（1024px）下顶栏标题过长导致的轻微溢出。
  - 全维度浏览器布局审计（8 个主流视口 × 7 种状态）达到 **ALL CLEAN — 0 issues** 全绿。

## [2.7.0] - 2026-08-20

### 中英双语阅读背景/双关解释框 + 四大多刊 20 篇新增

- **段落专属背景/双关/典故解释框（`.zh-annotation`）**：
  - 在中英对照阅读区域，若段落具备历史背景、政治时代背景、文化典故或语言双关，在 `.zh-text-card` 下方常驻渲染沉浸式解释框。
  - 采用卡片内嵌式浅色微透底色与虚线边框，动态绑定 `--issue-accent` 主题强调色。
  - 支持多类型徽章高亮前缀：`💡【时代背景】`、`🏛️【政治/历史背景】`、`🧩【语言双关】`、`🌍【文化典故】`。
  - 响应式完美适配：移动端（≤640px）内边距严格保持 24px 左对齐，字号随全局设置自适应缩放；纯英文视图下自动联动隐藏。
- **多源数据与 20 篇新刊采集与回填**：
  - 支持 **The Atlantic**、**The New Yorker**、**Wired** 与 **The Economist** 4 家顶级英文刊物。
  - 自动将 20 篇新文章采集整理至 `manual_source/`，并为全部 20 篇新文章及历史 8 篇文章生成 100% 全量精准翻译与深度背景解释侧车文件（`manual_translations/*.zh.json`）。
  - 自选文库扩充至 28 篇，全量回填翻译与时代背景注释。
...
- **杂志卡片序号**：期刊馆每张杂志卡片封面左上角新增序号角标（与自选角标共用 `.shelf-card-index` 样式），按当前展示顺序编号，便于按序定位。
- **「最新在前」翻转按钮（两者皆有）**：自选文库与杂志标题行各新增一个 `⇅ 最新在前 / ⇅ 最早在前` 翻转按钮。
  - 自选：默认按「Markdown 在前、草稿在后」的稳定序；翻转后按真实日期（`date` 字段，草稿用 id 内嵌时间戳兜底）**统一倒序**，最新文章置顶。
  - 杂志：按 `id`（YYYY-MM）排序；翻转后倒序，最新刊物置顶。
  - 两个翻转状态相互独立（`state.manualNewestFirst` / `state.magazineNewestFirst`，内存态），点击即时重渲染，序号随序同步。
- 新增分区标题行按钮组 `.shelf-title-actions`，导出与翻转按钮并列；移动端（≤640px）按钮组占满整行、两按钮均分，避免溢出。
- `src/core.js` 版本号 bump 至 2.6.18；`npm run build` + `npm run smoke` + `npm run lint` 均通过。

## [2.6.17] - 2026-08-10

### 书架门户 UX 改进（序号 / 隐藏内部标记 / 导出 / 折叠）

- **自选文章首页序号**：每张自选卡片封面左上角新增序号角标（与阅读器内 `ARTICLE NNN` 同源，由 `getManualArticleOrdinal` 计算，刷新后稳定），方便按序定位。
- **隐藏内部翻译状态**：移除卡片上的「已译 / 待译」标记（属内部信息，不应外显），现在只显示「Markdown · N 段」。
- **导出全部自选文章**：分区标题行新增「⤓ 导出全部自选」按钮，弹出格式选择（📄 双语 Markdown / 🇨🇳 纯中文 Markdown / 🧾 JSON），合并全部 Markdown 与草稿为单文件下载。
- **长列表折叠**：自选文库与杂志网格数量过多时自动收起（响应式阈值 **桌面 12 / 移动 6**），底部出现「显示全部 N 篇 ▾ / 收起」按钮；「＋新建」入口卡始终可见；窗口跨 640px 阈值时自动重算。
- `src/core.js` 版本号 bump 至 2.6.17；`npm run build` + `npm run smoke` + `npm run lint` 均通过。
...
- 2.6.11 把移动端中英左右内边距收窄到 17px 后，用户反馈英文段在带主题色强调框内**几乎贴到左侧边框线**，且中英文需严格左对齐避免层次混乱。
- 实测确认：中英 padding 已一致（都是 17px），但 17px 在 390 窄屏上距框线仅 18px，视觉偏紧。
- **修复**：移动端 `.en-text`/`.zh-text-card` 左右内边距统一回 **20px**（文字距强调框线 = 1px border + 20px = 21px，不再贴线；比桌面 22px 略紧凑；中英严格左对齐）。

## [2.6.11] - 2026-08-10

### 体验（移动端阅读区左右内边距再收窄）

- 2.6.10 把移动端中英左右内边距提回 20px 后，中文段（字更小）显得左右更空、英文正常段落也偏宽。本轮统一收窄到 **`17px`**（比 15px 略宽松防英文贴边、又比 20px 紧凑，中英文严格一致）。

## [2.6.10] - 2026-08-10

### 体验（移动端阅读区左右内边距修正）

- 2.6.8 为统一中英左右内边距、压缩垂直空行，把移动端 `.en-text`/`.zh-text-card` 左右内边距压到 `15px`；反馈英文段离左缘过近。本轮提回 **`20px`**（接近桌面端 22px 比例），中英文左右仍保持一致。
