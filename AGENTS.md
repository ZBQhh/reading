...
> **翻译工作全流程与每次批次登记**：详见 [`docs/translation-log.md`](docs/translation-log.md)。
> 该日志规定「每次只翻译/核对**本轮新增或改动**的文章，不重复核对历史已核验文章」，
> 并维护每篇文章的段数、图注数、译者、日期与核对状态表。新增/修改自选文章译文后，
> 务必在该日志状态表登记，避免后续 agent 重复全量核对。
